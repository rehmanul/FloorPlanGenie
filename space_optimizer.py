...
class SpaceOptimizer:
    ...
    def optimize_placement(self, plan_data, box_dimensions, corridor_width):
        ...
        dimensions = plan_data['dimensions'] # Renamed for clarity
        ...
        max_box_width = max(width * 0.15, 2.0)  # Minimum 2m, max 15% of space
        max_box_height = max(height * 0.15, 2.0)  # Minimum 2m, max 15% of space

        if box_dimensions['width'] > max_box_width:
            box_dimensions['width'] = max_box_width
        if box_dimensions['height'] > max_box_height:
            box_dimensions['height'] = max_box_height

        # Ensure minimum box size
        box_dimensions['width'] = max(box_dimensions['width'], 1.5)
        box_dimensions['height'] = max(box_dimensions['height'], 1.5)

        print(f"Adjusted dimensions: Space {width:.2f}x{height:.2f}m, Box {box_dimensions['width']:.2f}x{box_dimensions['height']:.2f}m")
        ...
        optimization_result = self._genetic_algorithm_placement(
            usable_bounds, box_dimensions['width'], box_dimensions['height'], corridor_width
        )
        ...
        statistics = self._calculate_detailed_statistics(
            optimization_result['boxes'], corridors, width, height, box_dimensions['width'], box_dimensions['height']
        )
